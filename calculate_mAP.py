...
def getHammingDist(code_a,code_b):
    ...
    for i in range(len(code_a)):
         if code_a[i]!=code_b[i]:
             dist += 1
    return dist/len(code_a)
def getCode_1(test_codes,test_groudTruth):
    line_number = 0
    with open(CURRENT_DIR+'/result_1.txt','r') as f:
        for line in f:
            temp = line.strip().split('\t')
            test_codes.append(temp[0])
            test_groudTruth.append(temp[1]) # get test ground truth(0-9)
            line_number += 1
    print(getNowTime(),'read data finish')
...
if __name__ =='__main__':
    ...
    total=0
    x=0
    getCode_1(test_codes,test_groudTruth)
    for i in range(len(test_codes)):
        for j in range(len(test_groudTruth)):
            x+=1
            dis=getHammingDist(test_codes[i],test_codes[j])<0.5
            if (test_groudTruth[i]==test_groudTruth[j])==dis:
                total+=1
            if j%10000==0:
                print('{}:{}%精确率'.format(getNowTime(),total/x))

    p_t=total/len(test_groudTruth)
